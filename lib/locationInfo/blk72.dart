...
class Blk72 extends StatefulWidget {
  ...
  @override
  State<Blk72> createState() => Blk72State();
}

class Blk72State extends State<Blk72> {
      ...
      body: SlidingUpPanel(
            ...
            child: SingleChildScrollView(
              child: Column(
                children: [
                  ...
                  Text("Current risk: " + getRisk(blk72Risk)),
                  Text("Average risk for the day: " +
                      AverageRisk['Blk 72'].toString()),
                  SfCartesianChart(
        ...
        collapsed: Container(
          decoration: BoxDecoration(
            color: (blk72Risk >= 1 && blk72Risk <= 3)
                ? Colors.green
                : (blk72Risk >= 4 && blk72Risk <= 7)
                    ? Colors.orange
                    : (blk72Risk >= 8 && blk72Risk <= 10)
                        ? Colors.red
                        : Colors.grey,
            borderRadius: radius,
          ),
    ...
    for (int i = 0; i < fullRiskData.length; i++) {
      if (dateNow == fullRiskData[i]["Date"].toString()) {
        if (fullRiskData[i]["Time"].substring(0, 2) != time) {
          ...
          chartData.add(RiskData(fullRiskData[i]["Blk 72"], int.parse(time)));
        }
      }
    }
