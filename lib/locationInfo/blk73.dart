...
class Blk73 extends StatefulWidget {
  ...
  @override
  State<Blk73> createState() => Blk73State();
}

class Blk73State extends State<Blk73> {
      ...
      body: SlidingUpPanel(
            ...
            child: SingleChildScrollView(
              child: Column(
                children: [
                  ...
                  Text("Current risk: " + getRisk(blk73Risk)),
                  Text("Average risk for the day: " +
                      AverageRisk['Blk 73'].toString()),
                  SfCartesianChart(
        ...
        collapsed: Container(
          decoration: BoxDecoration(
            color: (blk73Risk >= 1 && blk73Risk <= 3)
                ? Colors.green
                : (blk73Risk >= 4 && blk73Risk <= 7)
                    ? Colors.orange
                    : (blk73Risk >= 8 && blk73Risk <= 10)
                        ? Colors.red
                        : Colors.grey,
            borderRadius: radius,
          ),
    ...
    for (int i = 0; i < fullRiskData.length; i++) {
      if (dateNow == fullRiskData[i]["Date"].toString()) {
        if (fullRiskData[i]["Time"].substring(0, 2) != time) {
          ...
          chartData.add(RiskData(fullRiskData[i]["Blk 73"], int.parse(time)));
        }
      }
    }
