...
class Blk23 extends StatefulWidget {
  ...
  @override
  State<Blk23> createState() => Blk23State();
}

class Blk23State extends State<Blk23> {
      ...
      body: SlidingUpPanel(
            ...
            child: SingleChildScrollView(
              child: Column(
                children: [
                  ...
                  Text("Current risk: " + getRisk(blk23Risk)),
                  Text("Average risk for the day: " +
                      AverageRisk['Blk 23'].toString()),
                  SfCartesianChart(
        ...
        collapsed: Container(
          decoration: BoxDecoration(
            color: (blk23Risk >= 1 && blk23Risk <= 3)
                ? Colors.green
                : (blk23Risk >= 4 && blk23Risk <= 7)
                    ? Colors.orange
                    : (blk23Risk >= 8 && blk23Risk <= 10)
                        ? Colors.red
                        : Colors.grey,
            borderRadius: radius,
          ),
    ...
    for (int i = 0; i < fullRiskData.length; i++) {
      if (dateNow == fullRiskData[i]["Date"].toString()) {
        if (fullRiskData[i]["Time"].substring(0, 2) != time) {
          ...
          chartData.add(RiskData(fullRiskData[i]["Blk 23"], int.parse(time)));
        }
      }
    }
